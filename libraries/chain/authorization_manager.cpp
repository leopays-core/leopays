...
namespace eosio { namespace chain {
                                                           ...
                                                           )const
   {
      ...
      if( !linked_permission_name ) // if action is linked to lpc.any permission
         return;
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is lpc.any
