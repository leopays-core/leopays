...
class memory {
      ...
      static_assert(sizeof(control_block) <= wcb_allowance, "LeoPays VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(-cb_offset == EOS_VM_OC_CONTROL_BLOCK_OFFSET, "LeoPays VM OC control block offset has slid out of place somehow");
      static_assert(stride == EOS_VM_OC_MEMORY_STRIDE, "LeoPays VM OC memory stride has slid out of place somehow");
