    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                ...
                out = subprocess.run(
                    ['leopays-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("""Please provide:
                ...
                ex:
                python setup_eosio_tests.py ~/code/leopays-vm-test-wasms ~/code/leopays ~/wasm_spec_tests
              """)
        sys.exit(1)
    ...
    generator = os.path.join(GENERATOR_DIR, 'leopays_test_generator')
