...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(lpc.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(lpc.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      {
         create_account( N(lpc.token));
         set_code( N(lpc.token), contracts::eosio_token_wasm() );

         auto result = push_action(N(lpc.token), N(create), mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         result = push_action(N(lpc.token), N(issue), mutable_variant_object()
                 ("to",       eosio_token)
...
const name currency_tester::eosio_token = N(lpc.token);
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(lpc.token), expected.get_symbol(), N(lpc.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(lpc.token), N(transfer), mutable_variant_object()
         ("from", eosio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(lpc.token), N(transfer), mutable_variant_object()
                                    ("from", eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "LPC");
      BOOST_REQUIRE_EQUAL(SY(4,LPC), sys.value());
      BOOST_REQUIRE_EQUAL("4,LPC", sys.to_string());
      BOOST_REQUIRE_EQUAL("LPC", sys.name());
   ...
   {
      BOOST_CHECK_EXCEPTION(symbol malformed(SY(6,LpC)),
                            symbol_type_exception, fc_exception_message_is("invalid symbol: LpC"));
   }

   // invalid - contains lower case characters, exception thrown
   {
      BOOST_CHECK_EXCEPTION(symbol(5,"LpC"),
