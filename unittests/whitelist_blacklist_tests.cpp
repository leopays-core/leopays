...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(lpc.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(lpc.token), contracts::eosio_token_wasm() );
         chain->set_abi(N(lpc.token), contracts::eosio_token_abi().data() );
         chain->push_action( N(lpc.token), N(create), N(lpc.token), mvo()
              ( "issuer", "lpc.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(lpc.token), N(issue), N(lpc.token), mvo()
              ( "to", "lpc.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(lpc.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(lpc.token), N(alice)};
   test.init();

   test.transfer( N(lpc.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(lpc.token), N(bob)};
   test.init();

   test.transfer( N(lpc.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(lpc.token) );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(lpc.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   tester1.chain->push_action( N(lpc), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(lpc), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(lpc), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(lpc) && act.name == N(onblock) ) return;
