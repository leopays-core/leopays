...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(lpc.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create lpc.msig and lpc.token
        create_accounts({N(lpc.msig), N(lpc.token), N(lpc.ram), N(lpc.ramfee), N(lpc.stake), N(lpc.vpay), N(lpc.bpay), N(lpc.saving) });
        // Set code for the following accounts:
        //  - lpc (code: lpc.bios) (already set by tester constructor)
        //  - lpc.msig (code: lpc.msig)
        //  - lpc.token (code: lpc.token)
        // set_code_abi(N(lpc.msig), contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi(N(lpc.token), contracts::eosio_token_wasm(), contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        set_code_abi(N(lpc.msig),
                     contracts::eosio_msig_wasm(),
                     contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        set_code_abi(N(lpc.token),
                     contracts::eosio_token_wasm(),
                     contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for lpc.msig and lpc.token
        set_privileged(N(lpc.msig));
        set_privileged(N(lpc.token));

        // Verify lpc.msig and lpc.token is privileged
        const auto& eosio_msig_acc = get<account_metadata_object, by_name>(N(lpc.msig));
        BOOST_TEST(eosio_msig_acc.is_privileged() == true);
        const auto& eosio_token_acc = get<account_metadata_object, by_name>(N(lpc.token));
        BOOST_TEST(eosio_token_acc.is_privileged() == true);


        // Create LPC tokens in lpc.token, set its manager as eosio
        ...
        create_currency(N(lpc.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion LPC tokens to lpc.system
        issue(N(lpc.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(lpc.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("lpc"));
