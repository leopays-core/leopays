...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
...

   {
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@lpc.code.
      dtt_action dtt_act3;
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("LPC7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
