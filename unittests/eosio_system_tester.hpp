...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(lpc.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( N(lpc.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("lpc") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("lpc")) + get_balance(name("lpc.ramfee")) + get_balance(name("lpc.stake")) + get_balance(name("lpc.ram")) );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& lpcin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",lpcin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(lpc.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(lpc.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(lpc.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(lpc.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(lpc.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("lpc"), name("lpc.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(lpc.msig), contracts::eosio_msig_wasm() );
         set_abi( N(lpc.msig), contracts::eosio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(lpc.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total LPC supply to activate chain
      transfer( name("lpc"), name("alice1111111"), core_from_string("650000000.0000"), name("lpc") );
...
inline uint64_t M( const string& lpc_str ) {
   return core_from_string( lpc_str ).get_amount();
}
