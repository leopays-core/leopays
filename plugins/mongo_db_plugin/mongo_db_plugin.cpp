...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      //
      // Allow logging traces if node is a producer for testing purposes, so a single leopays-node can do both for testing.
      //
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine lpc setabi.abi from bytes to abi_def
   ...
   try {
      ...
      if( accounts.estimated_document_count() == 0 ) {
         ...
         try {
            ...
            //   1. enableSharding database (default to LEOPAYS)
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between leopays-node and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'LEOPAYS' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/LEOPAYS")
         ...
         ("mongodb-filter-on", bpo::value<vector<string>>()->composing(),
          "Track actions which match receiver:action:actor. Receiver, Action, & Actor may be blank to include all. i.e. lpc:: or :transfer:  Use * or leave unspecified to include all.")
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               EOS_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all LEOPAYS collections from mongodb." );
            }
         }
