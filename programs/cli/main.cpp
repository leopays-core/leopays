/**
  ...
  @section intro Introduction to leopays-cli

  `leopays-cli` is a command line tool that interfaces with the REST api exposed by @ref leopays-node. In order to use `leopays-cli` you will need to
  have a local copy of `leopays-node` running and configured to load the 'eosio::chain_api_plugin'.

   leopays-cli contains documentation for all of its commands. For a list of all commands known to leopays-cli, simply run it with no arguments:
```
$ ./leopays-cli
Command Line Interface to leopays-cli
Usage: programs/cli/leopays-cli [OPTIONS] SUBCOMMAND
...
Options:
  ...
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where leopays-wallet is running
...
//copy pasta from leopays-wallet's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "leopays-wallet" / (string(wallet_executable_name) + ".sock")).string();
...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      if(url == ::url)
         ...
         std::cerr << localized("Failed to connect to ${k} at ${u}; is ${k} running?", ("k", wallet_executable_name)("u", url)) << std::endl;
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "LPC") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
...
void try_local_port(uint32_t duration) {
   ...
   while ( !local_port_used()) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to " << wallet_executable_name << ", if " << wallet_executable_name << " is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to ${k}", ("k", wallet_executable_name))});
      }
   }
}
...
void ensure_keosd_running(CLI::App* app) {
    if (no_auto_keosd)
        return;
    // get, version, net, convert do not require leopays-wallet
    ...
    // This extra check is necessary when running leopays-cli like this: ./leopays-cli ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(wallet_executable_name); // if leopays-cli and leopays-wallet are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("leopays-wallet").append(wallet_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        pargs.push_back(string(wallet_executable_name) + ".sock");
        ...
        std::cerr << "No wallet service listening on "
                  << ". Cannot automatically start " << wallet_executable_name << " because " << wallet_executable_name << " was not found." << std::endl;
    }
}
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so leopays-cli can still work with old buggy leopays-node versions
                               // Change to voter.value when leopays-cli no longer needs to support leopays-node versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when leopays-cli no longer needs to support leopays-node versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "lpc")("scope", "lpc")("table", "namebids")
                               ("lower_bound", name(newname).to_uint64_t())
                               ("upper_bound", name(newname).to_uint64_t() + 1)
                               // Less than ideal upper_bound usage preserved so leopays-cli can still work with old buggy leopays-node versions
                               // Change to newname.value when leopays-cli no longer needs to support leopays-node versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      std::cout << "creator: " << string(res.creator.to_string()) << std::endl;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of leopays-node responding to the request is different than core symbol built into leopays-cli
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to leopays-cli"};
   ...
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where ${k} is running", ("k", wallet_executable_name)) )->group("");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where ${k} is running", ("k", wallet_executable_name)) )->group("");

   app.add_option( "-u,--url", url, localized("the http/https URL where ${n} is running", ("n", node_executable_name)), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where ${k} is running", ("k", wallet_executable_name)), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.add_flag( "--no-auto-wallet", no_auto_keosd, localized("don't automatically launch a ${k} if one is not currently running", ("k", wallet_executable_name)));
   ...
   getCode->callback([&] {
      ...
      try {
         ...
         //see if this is an old leopays-node that doesn't support get_raw_code_and_abi
...
//                                ->check(CLI::ExistingFile);
   ...
   auto stopKeosd = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", wallet_executable_name)));
   stopKeosd->callback([] {
      const auto& v = call(wallet_url, keosd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success leopays-wallet responds with empty object
   ...
   sign->add_option("--public-key", str_public_key, localized("Ask ${exec} to sign with the corresponding private key of the given public key", ("exec", wallet_executable_name)));
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(lpc.msig), N(propose), variant_to_bin( N(lpc.msig), N(propose), args ) }});
   });
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so leopays-cli can still work with old buggy leopays-node versions
                                 // Change to name(proposal_name).value when leopays-cli no longer needs to support leopays-node versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when leopays-cli no longer needs to support leopays-node versions older than 1.5.0
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(lpc.msig), name(action), variant_to_bin( N(lpc.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(lpc.msig), N(invalidate), variant_to_bin( N(lpc.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(lpc.msig), N(cancel), variant_to_bin( N(lpc.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(lpc.msig), N(exec), variant_to_bin( N(lpc.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send lpc.system contract action to the blockchain."));
