...
struct server_name_def {
  ...
  string leopays_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), leopays_home(), instances(1) {}
};
...
string producer_names::producer_name(unsigned int producer_number, bool shared_producer) {
   ...
   if (string(prod_name) == "defproducera" && producer_number != 0)
      throw std::runtime_error( "leopays-launcher not designed to handle numbers this large " );
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  char* erd_env_var = getenv ("LEOPAYS_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable LEOPAYS_HOME is set to the build path." << endl;
    exit (-1);
  }
  ...
  if (per_host == 0) {
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->leopays_home =
          (local_id.contains (lhost->host_name) || servers.default_leopays_home.empty()) ?
          erd : servers.default_leopays_home;
        host_ndx++;
      } // ph_count == 0
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->leopays_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->leopays_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->leopays_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->leopays_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->leopays_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::prep_remote_config_dir (eosd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->leopays_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->leopays_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->leopays_home;

  cmd = "cd " + host->leopays_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->leopays_home
...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  string eosdcmd = install_path + "programs/node/" + string(node_executable_name) + " ";
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->leopays_home + "; nohup " + eosdcmd + " > "
    ...
    string cmd = "cd " + host->leopays_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.leopays_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/leopays-tn_bounce.sh " + eosd_extra_args;
      ...
      do_command(host, node.name, { { "LEOPAYS_HOME", host.leopays_home }, { "LEOPAYS_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/leopays-tn_down.sh ";
      do_command(host, node.name,
                 { { "LEOPAYS_HOME", host.leopays_home }, { "LEOPAYS_NODE", node_num }, { "LEOPAYS_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/leopays-tn_roll.sh ";
      do_command(*host, host_name, { { "LEOPAYS_HOME", host->leopays_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  variables_map vmap;
  options_description cfg ("Testnet leopays-launcher config options");
  options_description cli ("leopays-launcher command line options");
  launcher_def top;
  string gts;
  launch_modes mode;
  string kill_arg;
  ...
  cli.add_options()
    ...
    ("kill,k", bpo::value<string>(&kill_arg),"The leopays-launcher retrieves the previously started process ids and issues a kill to each.")
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the leopays-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the leopays-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the leopays-tn_roll.sh script")
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(leopays_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (leopays_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_leopays_home) (ssh) )
