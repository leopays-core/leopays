...
function usage() {
  ...
  -i DIR      Directory where LeoPays is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the LeoPays data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export LEOPAYS_INSTALL_DIR=${INSTALL_LOCATION:-$LEOPAYS_INSTALL_DIR}

if [[ ! -d "${LEOPAYS_INSTALL_DIR}" ]]; then
   echo "[LeoPays installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${LEOPAYS_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home
   echo "[LeoPays installation found: ${LEOPAYS_INSTALL_DIR}]" && INSTALL_PATHS+=("${LEOPAYS_INSTALL_DIR}") # LEOPAYS_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${LEOPAYS_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/leopays")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/leopays")
            fi
            # Version < 1.8.0; Before we started using ~/leopays/1.8.x
            ...
            echo " - LeoPays Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled LeoPays Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing LeoPays Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
