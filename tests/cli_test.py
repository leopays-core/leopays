...
def node_help_test():
    """Test that leopays-node help contains option descriptions"""
    help_text = subprocess.check_output(["./programs/node/leopays-node", "--help"])
...
def cli_help_test(args):
    """Test that leopays-cli help contains option and subcommand descriptions"""
    help_text = subprocess.check_output(["./programs/cli/leopays-cli"] + args)
...
def cli11_bugfix_test():
    ...
    completed_process = subprocess.run(
        ['./programs/cli/leopays-cli', '-u', 'http://localhost:0/',
    ...
    assert(b'Failed to connect to leopays-node' in completed_process.stderr)
...
def cli11_optional_option_arg_test():
    ...
    output = subprocess.check_output(['./programs/cli/leopays-cli', 'sign',
                                      '-c', chain, '-k', '{}'],
                                     input=key.encode(),
