...
#
# Tests restart scenarios for leopays-node.  Uses "-c" flag to indicate "replay" (--replay-blockchain), "resync"
# (--delete-all-blocks), "hardReplay"(--hard-replay-blockchain), and "none" to indicate what kind of restart flag should
# be used. This is one of the only test that actually verify that leopays-node terminates with a good exit status.
#
###############################################################
...
try:
    ...
    if cluster.killSomeInstances(killCount, killSignal) is False:
        errorExit("Failed to kill LeoPays instances")
    Print("leopays-node instances killed.")
    ...
    if cluster.relaunchInstances(cachePopen=True) is False:
        errorExit("Failed to relaunch LeoPays instances")
    Print("leopays-node instances relaunched.")
    ...
    if killInstances:
        ...
        assert atLeastOne, "Test is setup to verify that a cleanly interrupted leopays-node exits with an exit status of 0, but this test may no longer be setup to do that"

    testSuccessful=True
finally:
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killInstances=killInstances, killWallet=killInstances, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
