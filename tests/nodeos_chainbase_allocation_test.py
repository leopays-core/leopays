...
try:
    ...
    # (automatically taken care by the automatically generated lpc account)
    ...
    cmd = "create account -j lpc sample LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\
         LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --delay-sec 600 -p lpc"
    trans = producerNode.processCliCmd(cmd, cmd, silentErrors=False)
    assert trans

    # Schedule a new producer to trigger new producer schedule for "global_property_object"
    newProducerAcc = Account("newprod")
    newProducerAcc.ownerPublicKey = "LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    newProducerAcc.activePublicKey = "LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    ...
    setProdsStr += ']}'
    cmd="push action -j lpc setprods '{}' -p lpc".format(setProdsStr)
    trans = producerNode.processCliCmd(cmd, cmd, silentErrors=False)
    assert trans
