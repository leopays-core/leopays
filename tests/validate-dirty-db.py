...
# validate-dirty-db
#
# Test for validating the dirty db flag sticks repeated leopays-node restart attempts
#
###############################################################
...
def runNodeAndGetOutput(myTimeout=3):
    """Startup leopays-node, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching leopays-node process.")
    cmd="programs/node/leopays-node --config-dir etc/leopays/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("leopays-node process launched.")

    output={}
    try:
        if debug: Print("Setting leopays-node process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("leopays-node process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: leopays-node is running beyond the defined wait time. Hard killing leopays-node instance.")
...
try:
    ...
    Print("Restart leopays-node repeatedly to ensure dirty database flag sticks.")
    timeout=6
    ...
    for i in range(1,4):
        ...
        if not ret[0]:
            errorExit("Failed to startup leopays-node successfully on try number %d" % (i))
        assert(ret[1])
