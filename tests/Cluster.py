...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="leopays-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is leopays-wallet running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="LEOPAYStest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet leopays-wallet running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: LeoPays server host
        port: LeoPays server port
        walletHost: LeoPays wallet host
        walletPort: LeoPays wallet port
        enableMongo: Include mongoDb support, configures LeoPays mongo plugin
        ...
        self.eosioAccount=self.defProducerAccounts["lpc"]= Account("lpc")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodeArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodeArgs=None, onlySetProds=False,
        ...
        extraNodeArgs: string of arguments to pass through to each nodoes instance (via --leopays-node flag on launcher)
        ...
        specificExtraNodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-leopays-node flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the lpc.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.NodeName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on leopays-node default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.LauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodeArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodeArgs += " --plugin eosio::wallet_api_plugin"
        if self.enableMongo:
            nodeArgs += " --plugin eosio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        ...
        if nodeArgs:
            cmdArr.append("--leopays-node")
            cmdArr.append(nodeArgs)
        ...
        if specificExtraNodeArgs is not None:
            ...
            for nodeNum,arg in specificExtraNodeArgs.items():
                ...
                cmdArr.append("--specific-leopays-node")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-leopays-node")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.NodeName, startedNodes, len(nodes)))
            return False
        ...
        self.eosioAccount=self.defProducerAccounts["lpc"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.CliPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.CliPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on LeoPays server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.NodeName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on LeoPays node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n LPC and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse LeoPays private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by leopays-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/lpc.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/lpc.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down LeoPays cluster.")
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"lpc\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init lpc transfer")
            opts="--permission lpc@active"
            if name != "lpc":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from lpc.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        """Create 'prodCount' init accounts and deposits 10000000000 LPC in each. If prodCount is -1 will initialize all possible producers.
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission lpc@active"
                    myTrans=biosNode.pushMessage("lpc", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("lpc", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but lpc).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "lpc"
        ...
        eosioTokenAccount.name="lpc.token"
        ...
        eosioRamfeeAccount.name="lpc.ramfee"
        ...
        eosioStakeAccount.name="lpc.stake"
        ...
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to lpc contract.")
            return None
        ...
        actualAmount=biosNode.getAccountBalanceStr(eosioAccount.name)
    ...
    @staticmethod
    def pgrepServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.NodeName)
    ...
    @staticmethod
    def pgrepServerPattern(nodeInstance):
    ...
    def discoverLocalNodes(self, totalNodes, timeout=None):
        ...
        psOut=Cluster.pgrepServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.NodeName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepServers(timeout=timeout)
        pattern=Cluster.pgrepServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.NodeName, pattern))
    ...
    # Kills a percentange of LeoPays instances starting from the tail and update InstanceInfos state
    def killSomeInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.NodeName, killSignal))
    ...
    def relaunchInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster leopays-node instances. allInstances will kill all leopays-node instances running on the system."""
        cmd="%s -k 9" % (Utils.LauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down LeoPays cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the LeoPays server
            cmd="pkill -9 %s" % (Utils.NodeName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down LeoPays cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces leopays-node instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down leopays-node instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
