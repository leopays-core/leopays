...
# --dump-error-details <Upon error print etc/leopays/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("lpc", "setprods", setProdsStr, opts)
...
try:
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, useBiosBootFile=False, onlySetProds=True, sharedProducers=1) is False:
        ...
        Utils.errorExit("Failed to stand up LeoPays cluster.")
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "lpc":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
