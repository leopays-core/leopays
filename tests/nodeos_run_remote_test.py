...
###############################################################
# node_run_remote_test
#
#  Tests remote capability of the node_run_test. Test will setup cluster and pass nodes info to node_run_test. E.g.
#  node_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
try:
    ...
    Print("Starting up %s test: %s" % ("leopays-node", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killInstances, False, False, killAll, dumpErrorDetails)
