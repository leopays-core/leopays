#!/bin/bash
# The purpose of this test is to ensure that the output of the "leopays-node --version" command matches the version string defined by our CMake files
# If the environment variable BUILDKITE_TAG is empty or unset, this test will echo success
echo '##### leopays-node Version Label Test #####'
...
# orient ourselves
[[ "$LEOPAYS_ROOT" == '' ]] && LEOPAYS_ROOT=$(echo $(pwd)/ | grep -ioe '.*/leopays/')
[[ "$LEOPAYS_ROOT" == '' ]] && LEOPAYS_ROOT=$(echo $(pwd)/ | grep -ioe '.*/leopays-core/leopays/')
[[ "$LEOPAYS_ROOT" == '' ]] && LEOPAYS_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using LEOPAYS_ROOT=\"$LEOPAYS_ROOT\"."
# determine expected value
CMAKE_CACHE="$LEOPAYS_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$LEOPAYS_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_LEOPAYS_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_LEOPAYS_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    set +e
    echo "LEOPAYS_ROOT=\"$LEOPAYS_ROOT\""
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_LEOPAYS_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_LEOPAYS_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$LEOPAYS_ROOT"'
    ls -la "$LEOPAYS_ROOT"
    echo '$ ls -la "$LEOPAYS_ROOT/build"'
    ls -la "$LEOPAYS_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get leopays-node version
ACTUAL=$($LEOPAYS_ROOT/build/bin/leopays-node --version) || : # leopays-node currently returns -1 for --version
# test
