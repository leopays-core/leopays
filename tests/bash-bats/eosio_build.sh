...
SCRIPT_LOCATION="scripts/build.sh"
TEST_LABEL="[leopays_build]"

#####################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL LeoPays BEFORE THESE WILL PASS #
#####################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        run bash -c "printf \"y\ny\nn\nn\n\" | ./scripts/build.sh"
        [[ ! -z $(echo "${output}" | grep "LeoPays compiler checks require the 'which'") ]] || exit
    fi
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "LEOPAYS_INSTALL_DIR: ${HOME}/leopays/${LEOPAYS_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s LPC2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: LPC2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "LEOPAYS_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "LEOPAYS_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
