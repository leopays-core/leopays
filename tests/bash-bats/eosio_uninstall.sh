...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - LeoPays Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled LeoPays Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i leopaystest
  [[ $output =~ .*/leopaystest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/leopays" ]] && [[ ! $output =~ ".local/share/leopays" ]]) || exit
  [[ ! $output =~ "LeoPays Removal Complete" ]] || exit
}
