...
killAll()
{
  programs/launcher/leopays-launcher -k 15
}

cleanup()
{
    rm -rf etc/leopays/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cli/leopays-cli get info)"
  verifyErrorCode "leopays-cli get info"
...
# stand up LeoPays cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --leopays-node \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/launcher/leopays-launcher --leopays-node "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cli/leopays-cli --port $port2 get block 1)"
    verifyErrorCode "leopays-cli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cli/leopays-cli create key)"
verifyErrorCode "leopays-cli create key"
...
PASSWORD_INITA="$(programs/cli/leopays-cli wallet create --name inita)"
verifyErrorCode "leopays-cli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cli/leopays-cli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "leopays-cli wallet import"
programs/cli/leopays-cli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "leopays-cli wallet import"
programs/cli/leopays-cli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "leopays-cli wallet import"
programs/cli/leopays-cli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "leopays-cli wallet import"
...
ACCOUNT_INFO="$(programs/cli/leopays-cli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "leopays-cli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cli/leopays-cli get account testera)"
verifyErrorCode "leopays-cli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cli/leopays-cli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "leopays-cli transfer"
   ...
   TRANS_INFO="$(programs/cli/leopays-cli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "leopays-cli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
