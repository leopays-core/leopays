...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="LEOPAYStest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def cliArgs(self):
        ...
        return self.endpointArgs + walletArgs + " " + Utils.MiscCliArgs
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getAccount(account.name, exitOnError=True)
            try:
    ...
    def getAccount(self, name, exitOnError=False, returnType=ReturnType.json, avoidMongo=False):
        ...
        if not self.enableMongo or avoidMongo:
            ...
            return self.getAccountFromDb(name, exitOnError=exitOnError)

    def getAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.CliPath, self.cliArgs(), source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def currencyStrToInt(balanceStr):
        """Converts currency string of form "12.3456 LPC" to int 123456"""
    ...
    def currencyIntToStr(balance, symbol):
        """Converts currency int of form 123456 to string "12.3456 LPC" where LPC is symbol string"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        """Validate each account has the expected LPC balance. Validate cumulative balance matches expectedTotal."""
        assert(source)
        assert(isinstance(source, Account))
        assert(accounts)
        assert(isinstance(accounts, list))
    ...
    # Get actions mapped to an account (leopays-cli get actions)
    ...
    def getAccountBalanceStr(self, scope):
        """Returns LPC currency0000 account balance from leopays-cli get table command. Returned balance is string following syntax "98.0311 LPC". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("lpc.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountBalance(self, scope):
        """Returns LPC currency0000 account balance from leopays-cli get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.CliPath, self.cliArgs(), account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.CliPath, self.cliArgs(), account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.CliPath, self.cliArgs(), account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def processCliCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        cmd="%s %s %s" % (Utils.CliPath, self.cliArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by leopays-cli get info."""
        if not self.enableMongo:
    ...
    # If nodePath is equal to None, it will use the existing leopays-node path
    def relaunch(self, nodeId, chainArg=None, newChain=False, timeout=Utils.systemWaitTimeout, addSwapFlags=None, cachePopen=False, nodePath=None):
    ...
    # Require PREACTIVATE_FEATURE to be activated and require lpc.bios with preactivate_feature
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("lpc", "activate", data, opts)
