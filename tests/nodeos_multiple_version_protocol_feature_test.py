...
#
# Test for verifying that older versions of leopays-node can work with newer versions of leopays-node.
#
###############################################################
...
try:
    ...
    # Finally, when we restart the 4th node with the version of leopays-node that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodePath="programs/node/leopays-node")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodePath="programs/node/leopays-node")
