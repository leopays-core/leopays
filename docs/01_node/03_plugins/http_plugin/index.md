...
The `http_plugin` is a core plugin supported by both `leopays-node` and `leopays-wallet`. The plugin is required to enable any RPC API functionality provided by a `leopays-node` or `leopays-wallet` instance.
...
```sh
# command-line
leopays-node ... --plugin eosio::http_plugin [options]
 (or)
leopays-wallet ... --plugin eosio::http_plugin [options]
```
...
Config Options for eosio::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=leopays-wallet.sock for leopays-wallet)
  --http-server-address arg (=127.0.0.1:8888 for leopays-node)
