---
content_title: leopays-node Plugins
---

## Overview

Plugins extend the core functionality implemented in `leopays-node`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `leopays-node`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | leopays-node is modular]]
| Plugins add incremental functionality to `leopays-node`. Unlike runtime plugins, `leopays-node` plugins are built at compile-time.
