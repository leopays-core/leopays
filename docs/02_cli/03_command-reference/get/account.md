...
### Get formatted data for user **lpc**

```sh
leopays-cli get account lpc
```
```console
privileged: true
permissions: 
...
producers:     <not voted>
```
### Get JSON data for user **lpc**

```sh
leopays-cli get account lpc --json
```
```json
{
  "account_name": "lpc",
  ...
  "permissions": [{
      ...
      "required_auth": {
        "threshold": 1,
        "keys": [{
            "key": "LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
            "weight": 1
          }
        ],
        "accounts": [],
