...
```sh
leopays-cli set action
```
**Output**

```console
Usage: leopays-cli set action [OPTIONS] SUBCOMMAND
...
```sh
leopays-cli set action permission
```
...
leopays-cli set action permission sandwichfarm lpc.system voteproducer voting -p sandwichfarm@voting

#Now can execute the transaction with the previously set permissions. 
leopays-cli system voteproducer approve sandwichfarm someproducer -p sandwichfarm@voting
```
