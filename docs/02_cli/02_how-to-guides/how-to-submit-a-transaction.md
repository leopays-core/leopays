...
[[info]]
| Be aware that if a clear text `data` field is used, leopays-cli need to fetch copies of required ABIs using `leopays-node` API. That operation has a performance overhead on `leopays-node`
...
```sh
leopays-cli push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
leopays-cli push transaction JSON
```
